...
class UserRequestModal extends Component {
  constructor (props) {
    super(props)
    this.state = {
      enteredVerification: '',
  ...
  componentDidMount () {
    ...
    $('#requestEdit-form').submit(() => {
      const isFormValid = $('#requestEdit-form').form('is valid')
      if (isFormValid) {
    ...
    return (
      <div ref={(n) => this.modal = n} className="ui modal">
        ...
        <div className="content">
          <form id="requestEdit-form" className="ui form">
...
const mapDispatchToProps = (dispatch) => ({
  onSub (state) {
    ...
    console.log(payload)
    fetch('http://localhost:8000/api/requests/'+state.requestID, {
      method: "PUT",
