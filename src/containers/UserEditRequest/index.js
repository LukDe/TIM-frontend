...
class UserEditRequest extends Component {
  constructor (props) {
    ...
    this.state = {
      ...
      misc: this.props.userTel,
      quantity: this.props.userMail,
      priority: this.props.userPostal,
      catastrophy: this.props.userPostal,
      postalCode: this.props.userPostal,
      creationDate: this.props.userPostal,
  ...
  componentDidMount () {
    $('.ui.form').form(userEditRequestValidation)
    $('#user-form').submit(() => {
  ...
  render (props) {
    return (
      <form id="user-form" className="ui form">
        ...
        <div className="field">
          <label><h1>Gesuch bearbeiten</h1></label>
        </div>

        <div className="fields">
          <label><h5>Was brauchen Sie?</h5></label>
          <div className="field">
        ...
        <div className="field">
          ...
          <input onChange={this.handleChange('catastrophy')}
                 type="text" name="catastrophy" value={this.state.catastrophy} />
        </div>

        <div className="fields">
          <label><h5>Dringlichkeit</h5></label>
          <div className="field">
            <div className="ui radio checkbox">
...
const mapStateToProps = (state) => ({
  ...
  goodName: R.path(['global', 'requests', 'data', '0', 'goodName'], state),
  misc: R.path(['global', 'requests', 'data', '0', 'misc'], state),
  quantity: R.path(['global', 'requests', 'data', '0', 'quantity'], state),
  priority: R.path(['global', 'requests', 'data', '0', 'priority'], state),
  catastrophy: R.path(['global', 'requests', 'data', '0', 'catastrophy'], state),
  postalCode: R.path(['global', 'requests', 'data', '0', 'postalCode'], state)
})
